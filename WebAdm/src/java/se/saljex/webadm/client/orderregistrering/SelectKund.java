...
import se.saljex.webadm.client.common.rpcobject.KundSuggestion;
 ...
 * @author Ulf
 */
public class SelectKund extends DialogBox{
	public static enum SEARCH_FIELD {
		KUNDNR, NAMN, SUPERSOK
	}
	...
	private SelectCallback<KundSuggestion> callback;
...
*/	
	ListWidget<KundSuggestion> listWidget = new ListWidget<KundSuggestion>(null, new PageLoad<KundSuggestion>(new KundSuggestion(), 10, 100, 1000, null) ,null) {

		@Override
		public void addListColumns(CellTable<KundSuggestion> cellTable) {
			getCellTable().addColumnStyleName(0, Const.Style_S10);
			getCellTable().addColumnStyleName(1, Const.Style_S20);
			TextColumn<KundSuggestion> c1 = new TextColumn<KundSuggestion>() {
				@Override
				public String getValue(KundSuggestion object) {
					return object.nummer;
				}
			};
			cellTable.addColumn(c1, "Nummer");

			c1 = new TextColumn<KundSuggestion>() {
				@Override
				public String getValue(KundSuggestion object) {
					return object.namn;
				}
	...
	public SelectKund(SelectCallback<KundSuggestion> callback) {
//		this.setSize("40em", Math.round(Window.getClientHeight()*0.5) + "px");
	...
	public void setCallback(SelectCallback<KundSuggestion> callback) {
		this.callback = callback;
	}
