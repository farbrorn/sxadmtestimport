...
if (!"true".equals(request.getParameter("inputform"))) {
	...
	PreparedStatement p = con.prepareStatement(
		"select f2.artnr, f2.namn, round(sum(f2.summa)/100)*100, round(sum(case f2.netto when 0 then 0 else f2.summa-f2.lev*f2.netto end)/100)*100, count(*), sum(f2.lev) "
		+" from faktura1 f1, faktura2 f2 where f1.faktnr = f2.faktnr "
	...
	%>
				<h1>Försäljningstoppen för <%= lagerNamn %></h1>
				Topplistan för sålda varor<br/>
				<table>
