<%-- 
    ...
    Created on : 2009-jul-27, 15:27:57
    Author     : ulf
--%>
...
if (frar == 0) frar = cal.get(Calendar.YEAR)-1;
...
if (!"true".equals(request.getParameter("inputform"))) {
	...
	PreparedStatement p = con.prepareStatement(
		"select f2.artnr, f2.namn, round(sum(f2.summa)/100)*100, " +
		" round(coalesce(sum(f2.summa - case when f2.netto = 0 then f2.summa else f2.lev*f2.netto + case when f1.bonus<>0 then f2.summa*fu.bonusproc2/100 else 0 end end ),0)/100)*100, count(*), sum(f2.lev) "
		+" from faktura1 f1, faktura2 f2 left outer join artikel a on a.nummer=f2.artnr, fuppg fu where a.lev='STE' and f1.faktnr = f2.faktnr "
		...
		+" and f2.artnr not like '*%' and f2.lev<>0"
		+" group by f2.artnr, f2.namn"
		+" order by " + sqlOrder
		+" limit " + maxrader
	...
	%>
				<h1>Försäljningstoppen sålda varor för <%= lagerNamn %></h1>
				<% if (lagerNr!=null) {
						if ("team".equals(lagerTyp)) {
							%>Rapporten visar försäljning för leverantör STE och inkluderar teamförsäljning från andra filialer<%
						} else {
							%>Rapporten visar försäljning för leverantör STE och endast försäljning över vald filial.<%
						}
					}
