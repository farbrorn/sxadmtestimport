<%-- 
    Document   : forsaljning-artgrupp
    Created on : 2009-feb-22, 09:28:14
    Author     : ulf
--%>
...
if (!"true".equals(request.getParameter("inputform"))) {
	...
	PreparedStatement p = con.prepareStatement(
		"select a.rabkod, coalesce(r.beskrivning,'Övrigt'), round(sum(f2.summa)/100)*100, round(sum(case f2.netto when 0 then 0 else f2.summa-f2.lev*f2.netto end)/100)*100, count(*), sum(f2.lev) "
		+" from faktura1 f1, faktura2 f2, artikel a left outer join rabkoder r on r.rabkod=a.rabkod and coalesce(r.kod1,'') = ''  where a.nummer=f2.artnr and f1.faktnr = f2.faktnr "
		+" and f1.datum between ? and ?"
		+" and " + lagerWhere
		+" and f2.artnr <> '*RÄNTA*' and f2.artnr <> '*BONUS*' and f2.lev<>0"
		+" group by a.rabkod, r.beskrivning"
		+" order by " + sqlOrder
			  );
	...
	%>
				<h1>Artikelgruppstoppen för <%= lagerNamn %></h1>
				Topplistan för artikelgrupper<br/>
				<table>
				...
				<table id="doclist">
					<tr>
						<th class="tds10">Grupp</th>
						<th class="tds30">Grupp</th>
