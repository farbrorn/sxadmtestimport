...
public class FakturaListaWidget extends SxWidget {
	...
	public FakturaListaWidget(GlobalData globalData, String headerText) {
		...
		ftColFormatter.addStyleName(1, globalData.STYLE_TD_ACTION);
		ftColFormatter.addStyleName(2, globalData.STYLE_TD_IDNR);
		ftColFormatter.addStyleName(3, globalData.STYLE_TD_DATUM);
		ftColFormatter.addStyleName(4, globalData.STYLE_TD_PRIS);
		ftColFormatter.addStyleName(5, globalData.STYLE_TD_MARKEINFO);
	...
	public void loadNextPage() {
		if (!blockTillFinishedLoading) {
			globalData.service.getFakturaHeaders(currentLoadedPage+1, pageSize, callackLoadPage);
	...
	private void visaClick(Anchor visaAnchor, FakturaHeader fakturaHeader, int row) {
		if ("Visa".equals(visaAnchor.getText())) {
			...
			//String st = ftRowFormatter.getStyleName(row);
			//ftRowFormatter.removeStyleName(row, st);
			//ftRowFormatter.setStyleName(row, globalData.STYLE_TR_HIGHLITE);
			//ftRowFormatter.setStylePrimaryName(row, globalData.STYLE_TR_HIGHLITE);
			//ftRowFormatter.addStyleName(row, st);
			//ftRowFormatter.setStyleName(row, globalData.STYLE_TR_HIGHLITE + " " + st);
			//ftRowFormatter.addStyleName(row, globalData.STYLE_TR_HIGHLITE);
			//ftRowFormatter.setStylePrimaryName(row, globalData.STYLE_TR_HIGHLITE);
	...
	private AsyncCallback callackLoadPage = new AsyncCallback() {
		...
		public void onFailure(Throwable caught) {
			ft.setWidget(currentRow, 0, new Label("Fel vid kommunikation."));
